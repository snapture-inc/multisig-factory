...
import { BUNDLER_URL, PAYMASTER_ADDRESS, PAYMASTER_URL, RPC_URL } from './constants';
 ...
 */
export function encodeSafeMintData(to: string, tokenId: bigint = getRandomUint256()): string {
...
function getRandomUint256(): bigint {
  ...
  for (let i = 0; i < dest.length; i++) {
    result |= BigInt(dest[i]) << BigInt(8 * i);
  }
