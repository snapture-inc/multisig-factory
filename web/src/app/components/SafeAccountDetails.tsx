            ...
            <Typography textAlign={'center'}>
              <Link href={safeLink} target="_blank" underline="hover" color="white">
                <Tooltip title={safeAddress}>
                  <Stack component={'span'} padding={4} direction={'row'} alignItems={'center'}>
                    <span style={{ margin: '0 8px' }}>{splitAddress(safeAddress)}</span>
                    <OpenInNewIcon />
                  </Stack>
                </Tooltip>
              </Link>
...
function splitAddress(address: string, charDisplayed: number = DEFAULT_CHAR_DISPLAYED): string {
